...
class inv_toolbar(inv):
    ...
    def update(self):
        ...
        if self.app.inv_recipe.is_open:
            return
        if self.app.inv_place_block.is_open:
            return

        mouse_status_type = self.app.mouse.status['action']
        mouse_status_button = self.app.mouse.status['button']
        if (mouse_status_type == MOUSE_TYPE_CLICK and mouse_status_button == MOUSE_LBUTTON) or (mouse_status_type == MOUSE_TYPE_DROP and mouse_status_button == MOUSE_LBUTTON):  # Rigth mouse button click
            if self.hover_item:
                rect = pg.Rect(pg.Vector2(self.get_pos(self.hover_cell_num))+self.inv_pos, self.inv_cell_size)
                if rect.collidepoint(self.app.mouse.status['rect'].topleft) and rect.collidepoint(self.app.mouse.status['rect'].bottomright):
                        ...
                        if self.hover_item['name'] == 'place_block':
                            if (not (self.app.inv_place_block.item is None) and (self.selected_cell_num == TOOL_PLACE)) or self.app.inv_place_block.item is None:
                                ...
                                if self.app.inv_place_block.item:
                                    self.app.mouse.setcursor_with_item(
                                ...
                                if self.app.inv_place_block.item:
                                    ...
                                    self.app.player.one_place_rect = None
                                    self.app.player.one_place_fit = None     
                                    self.app.player.place_rect = None
                                    self.app.player.place_fit = None     
    ...
    def load_from_data(self):
        ...
        for item in self.app.data.data['toolbar']:
            ...
            item['img'] = (pg.image.load(
                path.join(img_dir, item['icon'])).convert_alpha())
